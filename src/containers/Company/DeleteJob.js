...
class DeleteJob extends Component {
    ...
    render() {
        ...
        if (this.state.isError) {
            return (
                <p>Error deleting job</p>
            );
        }

        return (
            <div>
                <label htmlFor="titleConfirmation">Enter Job Title to delete:</label>
                <input type="text" name="titleConfirmation" onChange={this.handleChange} />
                <button type="button" onClick={this.handleSubmit}>Submit</button>
                <button type="button" onClick={this.props.toggleDeleteJob}>Cancel</button>
                {this.state.errorMsg}
            </div>
        );
    }
